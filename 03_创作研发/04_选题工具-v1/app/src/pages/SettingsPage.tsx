...
export function SettingsPage() {
  ...
  return (
    <div className="page page--settings">
      <PageHeader eyebrow="ADMIN / 运行设置" title="管理模型、采集与提示词" description="当前配置保存在本机，API Key 和 Cookie 会完整显示，方便直接核对和修改。" actions={<button className="button button--primary" type="submit" form="runtime-settings-form"><Save size={17} /> 保存设置</button>} />
        ...
        <section className="settings-panel">
          {tab === 'model' ? (
            <div className="settings-section">
              ...
              <div className="form-grid">
                <label className="span-2"><span>API Key</span><div className="secret-field"><KeyRound size={17} /><input type="text" value={draft.apiKey} onChange={(event) => setDraft({ ...draft, apiKey: event.target.value })} placeholder="输入 Ark 或兼容服务的 API Key" /></div></label>
              ...
              <div className="connection-note"><Cpu size={18} /><div><strong>统一配置</strong><p>本地所有分析任务使用这里的模型配置，页面会直接显示当前保存的完整内容。</p></div></div>
            </div>
          ) : null}
          ...
          {tab === 'crawler' ? (
            <div className="settings-section">
              ...
              <div className="cookie-status-grid"><div><span>当前长度</span><strong>{draft.douyinCookie.length}</strong></div><div><span>保存方式</span><strong>本地明文</strong></div><div><span>配置来源</span><strong>管理员手动粘贴</strong></div></div>
