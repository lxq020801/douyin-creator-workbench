...
版本：`v0.4.0-dev`。提示词包版本：`codex-topic-diversity-v1`。
...
- 用户与权限：服务端会话登录、管理员和普通用户角色、独立工作空间、管理员新增/删除用户及重设密码。
...
- `nginx:1.29-alpine`（前端静态文件与同源 API 代理）
- `redis:7.4-alpine`
- RQ 官方命令行 Worker
...
- 管理设置：`http://127.0.0.1:4191/admin/settings`（仅管理员）
...
```text
app/                 React + TypeScript + Vite 构建、Nginx 生产运行
...
前端通过同源 `/api` 请求后端。FastAPI 将耗时任务投递到 Redis/RQ；三个独立 Worker 处理单条分析、账号样本分析和脚本生成。SQLite 保存用户、会话、报告、资料、选题、脚本与任务状态，视频临时文件按保留时间清理。

每个用户拥有独立 `workspace_id`。升级前的 `local` 数据自动归入初始管理员工作空间，普通用户无法通过接口读取或修改其他工作空间的内容。模型、抖音 Cookie 和提示词属于系统运行配置，只允许管理员访问。
...
当前不包含服务器部署、域名和 HTTPS、自助注册、找回密码邮件、付费额度、Cookie 自动同步、自动发布、发布后数据回流、其他平台和本地上传。详情见 [CHANGELOG.md](CHANGELOG.md)。
