...
【核心任务】
以对标视频的核心创作方法为基因，结合用户账号的真实业务、受众、资源与目标，生成20个具有明显差异、且清晰继承对标价值的选题，完成从“对标方法”到“自身选题”的迁移。允许部分选题共享对标方法、受众、素材或表达形式，但不能让同一个核心创意占据大多数。
...
用一段话说明：20个选题分别从哪些受众处境、业务切口、内容功能、表达形式或具体场景展开，以及整体如何避免大多数选题只是同一内容的换皮变体。

【执行与自检规则】

1. 所有选题必须清晰对应对标拆解中的创作方法，不得脱离对标自由创作。

2. 允许部分选题共享同一创作方法、受众、素材或表达形式，不要求每条都完全不同。但整体必须在受众处境、业务切口、内容功能、表达形式或具体场景上形成变化；禁止整体换皮，不能只替换人物、产品、数字、地点等表层信息，而核心问题、内容逻辑和观看价值基本不变。

3. 不得将对标视频中的案例、数据、服务、价格等直接安到用户账号上，所有内容必须基于资料卡中的真实资源。

4. 生成时逐条比较选题的核心问题、内容逻辑和观看价值。局部相似可以保留，但相同的核心问题、叙事逻辑和观看价值不能占据大多数；如果出现这种情况，必须主动改成不同方向。标题可以使用自然、有效的常见表达方式，但不要让20条大量使用同一种开头或句式。
